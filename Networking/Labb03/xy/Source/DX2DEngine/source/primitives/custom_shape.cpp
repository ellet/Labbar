...
int CCustomShape::AddPoint(DX2D::Vector2f aPoint, CColor aColor)
{
	SCustomPoint point;
...
void DX2D::CCustomShape::Render()
{
	if (myIsDirty)
	{
		INFO_PRINT("%s", "CCustomShape::Render() shape is dirty, please call CCustomShape::BuildShape() to veryfy shape");
		return;
	}
