...
void Go( void )
{
	...
	DX2D::CSprite sprite("sprites/tga_logo.dds");

	// Setting the pivot so all operations will be in the middle of the image (rotation, position, etc.)
	sprite.SetPivot(DX2D::Vector2f(0.5f, 0.5f));


	/////////// Custom shader

	// Create a new shader
	DX2D::CCustomShader customShader; // Create
	customShader.SetShaderdataFloat4(DX2D::Vector4f(1, 0, 1, 1), DX2D::EShaderDataID_1); // Add some data to it
	customShader.SetTextureAtRegister(DX2D::CEngine::GetInstance()->GetTextureManager().GetTexture("sprites/tga_logo.dds"), DX2D::EShaderTextureSlot_1); // Add a texture

	// Run PostInit to set all the data
	customShader.PostInit("../source/tutorials/Tutorial3Customshader/custom_sprite_vertex_shader.fx", "../source/tutorials/Tutorial3Customshader//custom_sprite_pixel_shader.fx", DX2D::EShaderDataBufferIndex_1);
	...
	{
		timer += 1.0f / 60.0f;
		if (!DX2D::CEngine::GetInstance()->BeginFrame(DX2D::CColor(0, 0, 0.3f, 1)))
		{
			break;
		}

		// Set a new position
		sprite.SetPosition(DX2D::Vector2f( (cos(timer) + 1) / 2, (sin(timer) + 1) / 2));
		// Set the rotation
		...
		sprite.SetPosition(DX2D::Vector2f((sin(timer) + 1) / 2, (cos(timer) + 1) / 2));
