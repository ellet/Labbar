...
namespace Kasino 
{
	namespace General 
	{
		...
		{
			if (aCheckCashFlow.GetTableFeelingsState() == enumTableLikeStates::eHate || (aThePlayer.GetMoney() <= 0))
			{
				return false;
			}
		...
		bool GiveInstructions(std::string aWelcome, std::string aInstructions, enumTableLikeStates aTableStanding)
		{
			...
			if (aTableStanding == enumTableLikeStates::eHate)
			{
				return false;
			}
			else if (aTableStanding == enumTableLikeStates::eLike)
			{
				std::cout << "Welcome back friend!" << std::endl;
			}
			else if (aTableStanding == enumTableLikeStates::eDislike)
			{
		...
		void PrintWinOrLose(const float aBetAmount, const float aGameOdds, const bool aUserWin, Player &aThePlayer, CashFlow & aTableWinnings)
		{
			...
			aThePlayer.ChangeMoney(moneyChange);
			aTableWinnings.ChangeCurrentFlow(moneyChange);
			std::cout << "You now have " << aThePlayer.GetMoney() << std::endl << std::endl;

			float const currentTableStandings = aTableWinnings.GetTableWinningsValue();
			if (currentTableStandings > 0)
			{
