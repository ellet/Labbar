...
namespace CommonUtilities
{
	...
	void TimeManager::DoUpdate()
	{
		if (myImPaused == false)
		{
			...
			myTimeDifference.QuadPart = (myCurrentStamp.QuadPart - myCountFromStamp.QuadPart) / (myFrequency);
			myDeltaTime = (myCurrentStamp.QuadPart - myPreviousStamp.QuadPart) / (myFrequency);
	...
	{
		return static_cast<float>(GetInstance().myDeltaTime / 1000000.0);
	}
	...
	{
		return  (static_cast<double>(GetInstance().myTimeDifference.QuadPart) / 1000000.0);
	}
