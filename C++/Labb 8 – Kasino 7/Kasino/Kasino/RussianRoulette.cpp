...
void RussianRoulette::PlayGame(Player & aThePlayer)
{
	if (myIsDealerAlive == true)
	{

		bool playAgain = Kasino::General::GiveInstructions("Welcome " + aThePlayer.GetPlayerName() + ", to the Russian Roulette table", "We will use a 8-chambered revolver. each turn we will spin the barrel and then aim at your own head and fire if you do not you lose the money you bet", myCashFlow.GetTableFeelingsState());
		...
		if (playAgain)
		{
			...
			} while (myIsDealerAlive == true && aThePlayer.GetAlive() == true);

			if (userWin || (myIsDealerAlive == true && aThePlayer.GetAlive() == true))
			{
