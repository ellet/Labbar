...
void Game::PlayerEnterCombat()
{
	if (GetCurrentRoom().IHaveCreatures() == true)
	{
		...
		combat::Combat(myPlayer, tempCreature);
...
void Game::ShowSpellBook()
{
	...
	tempMenu.push_back("Memorize spell for combat");
...
void Game::GiveShortCutSpellOptions()
{
	do
	{
		const size_t SpellToAdd = ChooseSpellInSpellBook();

		if (SpellToAdd < myPlayer.GetSpellBookSize())
		{
			if (myPlayer.CheckIfSpellInCastableAttacks(SpellToAdd) == false)
			{
				myPlayer.AddCastableAttack(SpellToAdd);
			}
			else
			{
				l1functions::console::PrintMessageWithPause("You have already memorized this spell.");
			}
		}
		else
		{
