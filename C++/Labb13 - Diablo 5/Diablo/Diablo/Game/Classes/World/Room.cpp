...
std::vector<const Item*>& Room::GetRoomItems()
{
	return myItems;
}
...
{
	if (aItemToAddToRoom != nullptr)
	{
		myItems.push_back(aItemToAddToRoom);
	}
	else
	{
		l1functions::console::PrintErrorToConsole("Trying to add null pointer to room items");
	}
}
...
const Item* Room::ItemPickedUp(unsigned int aItemIndex)
{
