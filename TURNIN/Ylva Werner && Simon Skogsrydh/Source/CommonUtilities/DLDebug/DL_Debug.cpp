...
bool Debug::Create(const std::string& aFile)
{
	...
	std::string aTempString = aFile + "/CrasLogs";
	const LPCTSTR CrashLogsFolder = aTempString.c_str();
	...
	aTempString = aTempString + "/" + temp;

	ourInstance->myOutput = std::ofstream(debugLogs, std::ofstream::out);
	ourInstance->myCrashOutputLocation = aTempString;
	return true;
}
