...
std::string Tga2D::CTextService::GetRenderName(const std::string& aFontPathAndName, EFontSize aSize)
{
	std::stringstream key;
...
bool CTextService::UpdateTextAndDraw(const std::string& aText, const Vector2f& aPosition, CColor aColor, float aSize, const std::string& aName, ESamplerType aSamplerType)
{
	...
	BuildText(aText, aColor, aSize, aName, fontData->myBufferedObjects);
...
void CTextService::BuildText(const std::string& aText, CColor aColor, float aSize, const std::string& aName, SSimplePointerArray<STextToRender*>& aTextBuffer)
{
	SFontData* fontData = myFontData[aName];
	if (!fontData)
	{
