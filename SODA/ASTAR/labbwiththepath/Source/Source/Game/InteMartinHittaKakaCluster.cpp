...
void InteMartinHittaKakaCluster::Update()
{
	...
	if (myIsFindingPath == true)
	{
		bestSolution = &myPath;
		size_t currentNode;
		myIsFindingPath = myCluster.AStar(myPath, currentNode); //Dijkstras(myPath, myGoalNodeID, currentNode);
...
void InteMartinHittaKakaCluster::GenerateMap(const size_t nodeAmount, const Vector2f & aWorldConstraints)
{
	...
	for (size_t iNode = 0; iNode < nodeAmount; ++iNode)
	{
		...
		//RandomizeMap(aWorldConstraints);
		CreateGridMap(iNode);
	}
}

void InteMartinHittaKakaCluster::CreateGridMap(const size_t aNodeIndex)
{
	size_t columns = 13;
	size_t spacing = 70;
	size_t offset = 100;
	...
	Vector2ui tempPosition;
	tempPosition.x = (aNodeIndex % columns) * spacing + offset;
	tempPosition.y = (aNodeIndex / columns) * spacing + offset;
	
	Vector2f floatyPosition = Vector2f(static_cast<float>(tempPosition.x), static_cast<float>(tempPosition.y));

	myGraphicalNodes.GetLast().SetPosition(floatyPosition);
	myCluster.SetNodePosition(aNodeIndex, floatyPosition);

	if (myGraphicalNodes.Size() > 1)
	{
		if ((aNodeIndex % columns) > 0)
		{
			myCluster.CreateLink(aNodeIndex -1, aNodeIndex);
		}
		if ((aNodeIndex / columns) > 0)
		{
			myCluster.CreateLink(aNodeIndex - columns, aNodeIndex);
		}
	}
}

void InteMartinHittaKakaCluster::RandomizeMap(const Vector2f & aWorldConstraints)
{
...
void InteMartinHittaKakaCluster::PrintNodeValues(Renderer & aRenderer)
{
	for (size_t iNode = 0; iNode < myGraphicalNodes.Size(); ++iNode)
	{
		...
		float tempValue = myCluster.GetNodeValue(iNode);

		if (tempValue != FLT_MAX)
		{
...
void InteMartinHittaKakaCluster::FindPath()
{
	...
	myCluster.StartAStar(myStartNodeID, myGoalNodeID);
...
bool InteMartinHittaKakaCluster::SetStartNodeWithMouse(const Vector2f & aMouseClickedPosition)
{
	...
	if (startID != SIZE_MAX && myCluster.CheckNodeBlock(startID) == false)
	{
...
bool InteMartinHittaKakaCluster::SetGoalNodeWithMouse(const Vector2f & aMouseClickedPosition)
{
	...
	if (goalID != SIZE_MAX && goalID != myStartNodeID && myCluster.CheckNodeBlock(goalID) == false)
	{
		myGoalNodeID = goalID;
		myGraphicalNodes[myGoalNodeID].IsGoal();
		return true;
...
void InteMartinHittaKakaCluster::SwitchBlockedNodeWithMouse(const Vector2f & aMouseClickedPosition)
{
	size_t blockedID = ClickedNode(aMouseClickedPosition);
	if (blockedID != SIZE_MAX)
	{
		myCluster.SwitchNodeBlocked(blockedID);
		myGraphicalNodes[blockedID].SwitchBlocked(myCluster.CheckNodeBlock(blockedID));
	}
}
