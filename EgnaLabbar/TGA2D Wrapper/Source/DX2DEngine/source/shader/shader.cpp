...
void DX2D::CShader::DoOneFrameUpdates(CRenderObject* aObject)
{
	...
	dataPtrCommon->myTimings.x = 0;
	dataPtrCommon->myTimings.y = 0;
	...
	for (unsigned int i = 0; i < lights.size(); i++)
	{
		...
		dataPtrLights->myLights[i].myLightPositions.Set(((lights[i]->myPosition.x + 0.5f) - worldPos.x) * myEngine->GetWindowSize().x, ((lights[i]->myPosition.y + 0.5f) - worldPos.y) * myEngine->GetWindowSize().y, lights[i]->myIntensity, lights[i]->myFallOff);
	}
