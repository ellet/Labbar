...
bool Debug::Create(const std::string& aFile)
{
	...
	std::string crashFolder = "CrashFolder";
	crashFolder = aFile + "/" + crashFolder;

#ifdef UNICODE
	std::wstring tempString(aFile.begin(), aFile.end());
	std::wstring tempCrashFolderString(crashFolder.begin(), crashFolder.end());
#else
	std::string tempString = aFile;
	std::string tempCrashFolderString = crashFolder;

#endif

	std::string aFolderName = crashFolder;

	const LPCTSTR DebugLogsFolder = tempString.c_str();
	const LPCTSTR CrashLogsFolder = tempCrashFolderString.c_str();
	CreateDirectory(DebugLogsFolder, NULL);
	CreateDirectory(CrashLogsFolder, NULL);
	...
	std::string debugLogs = tester + "/" + temp;
	aFolderName = aFolderName + "/" + temp;

	ourInstance->myOutput = std::ofstream(debugLogs, std::ofstream::out);
	ourInstance->myCrashOutputLocation = aFolderName;
	return true;
}
