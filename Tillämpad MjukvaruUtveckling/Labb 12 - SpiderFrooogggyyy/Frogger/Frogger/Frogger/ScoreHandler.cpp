...
{    
	...
	*/
	...
	MainSingleton::GetInstance()->GetRenderer().AddTextRenderCommand(*myFont, CommonUtilities::Vector2f(10, 448), HGETEXT_LEFT, score);
    ...
    for(int index = 0; index < frogLives; index++)
    {
		...
		CommonUtilities::Vector2f first(static_cast<float>(10 + 20 * index), 430.0f);
		CommonUtilities::Vector2f second(static_cast<float>(30 + 20 * index), 450.0f);

		MainSingleton::GetInstance()->GetRenderer().AddStretchedRenderCommand(*sprite, first, second);
    }
    ...
    for(int index = 0; index < 5; index++)
    {
        if(myFrogHomes[index].myIsTaken)
        {
			...
			MainSingleton::GetInstance()->GetRenderer().AddRenderCommand(*sprite, myFrogHomes[index].myPosition);
        }
