...
namespace CommonUtilities
{
	namespace Math
	{
		inline int CapValue(int aMinValue, int aMaxValue, int aValueToCap)
		{
		...
		inline float CapValue(const float aMinValue, const float aMaxValue, float aValueToCap)
		{
			if (aValueToCap > aMaxValue)
			{
				aValueToCap = aMaxValue;
		...
		inline float Saturate(const float aValue)
		{
