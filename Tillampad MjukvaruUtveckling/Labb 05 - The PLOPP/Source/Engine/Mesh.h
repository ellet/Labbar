...
public:
	...
	void SetTexture(const std::shared_ptr<Texture> & aTexture, const unsigned int aTextureIndex);

	void Render() const;
	void RenderInstanced(int aInstanceCount) const;

	std::shared_ptr<Texture> GetTexture(const unsigned int aTextureIndex);
...
private:
	...
	int myVertexCount;
	int myIndexCount;
	std::array<std::shared_ptr<Texture>, 11> myTextures;
