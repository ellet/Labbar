 ...
 *  maxium frame rate.
 */
float MainLoop::getLimitedDtInSeconds()
{
    ...
    while( 1 )
    {
        ...
        const int current_fps = (int)(1.0f/dt);
...
void MainLoop::run()
{
    ...
    while(!m_abort)
    {
        ...
        m_prev_time = m_curr_time;
        float dt   = getLimitedDtInSeconds();

		/*std::string temp = "Delta time is: " + std::to_string(dt);

		Log::verbose("main", temp.c_str());*/
