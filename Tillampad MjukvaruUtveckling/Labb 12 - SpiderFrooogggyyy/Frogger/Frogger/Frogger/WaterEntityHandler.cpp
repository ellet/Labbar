...
{	
	WaterEntity * tempWaterEntity = nullptr;

	for(unsigned int index = 0; index < myWaterEntitysIngame.Count(); index++)
	{
		if(myWaterEntitysIngame[index]->HasCollided(aFrog->GetMoveToPosition(), 0))
		{
			tempWaterEntity = myWaterEntitysIngame[index];
		}
	}

	aFrog->SetCurrentWaterEntity(tempWaterEntity);
}
