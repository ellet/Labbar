...
float4 MotionBlur(PixelInput aInput) : SV_Target
{
    float numberOfSamples = 20.f;
    float blurSize = 0.2f;

    float2 resourceTwo = boundSecondTexture.SampleLevel(samplerState, aInput.uv, 0).rg;
    ...
    for (int indexBuffer = 0; indexBuffer < numberOfSamples; ++indexBuffer)
    {
        float3 currentColor = boundTexture.SampleLevel(samplerState, textureSample, 0).rgb;
