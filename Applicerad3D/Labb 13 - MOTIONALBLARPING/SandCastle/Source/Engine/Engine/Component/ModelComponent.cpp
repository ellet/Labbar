...
namespace ENGINE_NAMESPACE
{
	...
	void ModelComponent::Render() const
	{
		...
		myModel->Render(myEffect, myMatrix * GetGameObject().GetTransformation(), myOldWorld, myColor);
		*const_cast<Matrix44f*>(&myOldWorld) = myMatrix * GetGameObject().GetTransformation();
	}
	...
	{
		myOldWorld = myMatrix * GetGameObject().GetTransformation();
	}
	...
	{
		GetGameObject().RotateAlongAxis(Vector3f::UnitY, -1.f * aDeltaTime.InSeconds());
	}
