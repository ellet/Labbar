	...
	ENGINE_NAMESPACE
{
	...
	{
		myIsCursorHidden = aIsHidden;
	}

	bool WindowsWindow::IsCursorHidden() const
	{
		return myIsCursorHidden;
	}
	...
	{
		if (myIsCursorConfined && myHasFocus)
		{
			RECT clientRect,
				innerRect;
		...
		// std::cout << (myIsCursorHidden == false || myHasFocus == false) << std::endl;
		// bool showCursor = myIsCursorHidden == false || myHasFocus == false;
		// SetCursor(showCursor ? IDC_HAND : nullptr);
	}
}
