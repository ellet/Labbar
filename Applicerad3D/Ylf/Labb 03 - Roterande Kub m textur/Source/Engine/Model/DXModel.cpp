...
void CDXModel::InitIndices()
{
	...
	myIndexes.Init(myVertices.Size());
	myIndexes.Resize(myVertices.Size());

	for (unsigned int i = 0; i < static_cast<unsigned int>(myIndexes.Size()); ++i)
	{
		myIndexes[static_cast<unsigned short>(i)] = i;
	}
}
...
void CDXModel::RenderModel()
{
	...
	// Render the model.
	tempDeviceContextRef.DrawIndexed(myIndexes.Size(), 0, 0);
}
