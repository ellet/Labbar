...
void PlayerShootComponent::Initialize()
{
	...
	myKeySubscription = std::make_shared<SB::Subscription<SB::LeftMouseButtonDownMessage>>(
		[this] (const SB::LeftMouseButtonDownMessage & aMessage) -> SB::ReceiveResult
		{
			return this->Receive(aMessage);
		}
	);
	myKeyUpSubscription = std::make_shared<SB::Subscription<SB::LeftMouseButtonUpMessage>>(
...
SB::ReceiveResult PlayerShootComponent::Receive(const SB::KeyDownMessage<SB::KeyboardKey::eZ>& aMessage)
{
	return SB::ReceiveResult::eContinue;
}
