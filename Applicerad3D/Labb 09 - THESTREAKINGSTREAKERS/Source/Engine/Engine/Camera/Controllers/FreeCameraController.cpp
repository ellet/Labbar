...
namespace ENGINE_NAMESPACE
{
	...
	CameraControllerResult FreeSpaceCameraController::Update(const Time & aDeltaTime, Camera & aCamera)
	{
		...
		aCamera.GetOrientation().RotateY(-myRotationDelta.x);
		aCamera.GetOrientation().RotateX(-myRotationDelta.y);
		myRotationDelta = Vector2f::Zero;
	...
	ReceiveResult FreeSpaceCameraController::Receive(const MouseMovedMessage& aMessage)
	{
