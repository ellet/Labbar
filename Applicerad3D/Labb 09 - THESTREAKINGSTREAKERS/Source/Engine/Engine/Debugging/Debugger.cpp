...
namespace ENGINE_NAMESPACE
{
	Debugger::Debugger(void * aWindowHandle)
	{
		...
		mySphereModel = std::make_shared<AssimpModel>("models/lightspheres/lightsphere_1.fbx");
	...
	void Debugger::RenderSphere(const Vector3f & aPosition, const float aRadius)
	{
		RenderSphere(aPosition, Vector3f::One * aRadius);
	}

	void Debugger::RenderSphere(const Vector3f & aPosition, const Vector3f &aRadius)
	{
		DrawShape(mySphereModel, aPosition, aRadius);
	}

	void Debugger::RenderCube(const Vector3f & aCenterPosition, const Vector3f &aSize)
	{
		DrawShape(myCubeModel, aCenterPosition, aSize);
	}

	void Debugger::DrawShape(const std::shared_ptr<Model> & aShape, const Vector3f & aPosition, const Vector3f & aScale)
	{
		Engine::GetRenderer().EnableWireframe();
		aShape->InstantRender(myWireframeEffect, SB::Matrix44f::CreateScale(aScale.x * 1.0f, aScale.y * 1.0f, aScale.z * 1.0f) * SB::Matrix44f::CreateTranslation(aPosition));
