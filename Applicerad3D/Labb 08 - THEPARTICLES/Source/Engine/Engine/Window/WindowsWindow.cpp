...
ENGINE_NAMESPACE
{
	...
	void WindowsWindow::ProcessMouseMoveData(WindowMessage& aMessage, int aX, int aY)
	{
		...
		// 	{
		// 		if (aMessage.data.mouseEvent.x != myPreviousMouseMoveMessage.data.mouseEvent.x || aMessage.data.mouseEvent.y != myPreviousMouseMoveMessage.data.mouseEvent.y)
		// 		{
	...
	void WindowsWindow::SetCursorLocked(bool aIsCursorLocked)
	{
		myIsCursorLocked = aIsCursorLocked;
	}

	bool WindowsWindow::IsCursorLocked() const
	{
		return myIsCursorLocked;
	}
