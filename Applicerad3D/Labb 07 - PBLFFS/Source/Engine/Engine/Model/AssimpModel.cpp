...
AssimpModel::AssimpModel(const std::shared_ptr<Effect> & aEffect, const std::string & aFilePath)
{
	...
	const int specularSlot = 1;
	const int ambientSlot = 2;
	const int emissiveSlot = 3;
	const int normalSlot = 5;
	const int reflectionSlot = 10;

	TryLoadTexture(*model, surface, modelDirectory, diffuseSlot, SurfaceTextureIndex::eDiffuse);
	TryLoadTexture(*model, surface, modelDirectory, specularSlot, SurfaceTextureIndex::eSpecular);
	TryLoadTexture(*model, surface, modelDirectory, ambientSlot, SurfaceTextureIndex::eAmbientOcclusion);
	...
	TryLoadTexture(*model, surface, modelDirectory, reflectionSlot, SurfaceTextureIndex::eMetalness);
