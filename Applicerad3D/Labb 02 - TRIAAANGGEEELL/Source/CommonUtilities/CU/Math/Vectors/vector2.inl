...
namespace CommonUtilities
{
	...
	template<typename T>
	void CommonUtilities::Vector2<T>::SetWithAngle(const T aAngleInDegrees, const T aDistance /*= static_cast<T>(1)*/)
	{
		x = static_cast<T>( cos(DEGRESS_TO_RADIANSF(FLOATCAST( aAngleInDegrees)))) * aDistance;
		y = static_cast<T>(sin(DEGRESS_TO_RADIANSF(FLOATCAST(aAngleInDegrees)))) * aDistance;
	}

	template<typename T>
	void CommonUtilities::Vector2<T>::SetWithAngleRadians(const T aAngleInRadians, const T aDistance /*= static_cast<T>(1)*/)
	{
		x = static_cast<T>(cos(FLOATCAST(aAngleInRadians))) * aDistance;
		y = static_cast<T>(sin(FLOATCAST(aAngleInRadians))) * aDistance;
	}
}
/*
