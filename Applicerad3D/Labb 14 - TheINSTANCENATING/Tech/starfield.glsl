...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float mySpeed = 0.2;
    float myStarLength = 1.0;
    float myAlpha = 1.0;
    
	float offset = iGlobalTime * mySpeed;	
	float speed2 = myStarLength; // (cos(offset)+1.0)*2.0;
	float speed = speed2+0.4;
	offset += offset*.96;
	offset *= 2.0;
	...
	fragColor = vec4(ToGamma(col) * myAlpha, 1.0);
}
