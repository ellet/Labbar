...
namespace ENGINE_NAMESPACE
{
	...
	void ColliderComponent::SetColliderLayerID(const CollisionFilter & aLayedID)
	{
	...
	ColliderData ColliderComponent::GetColliderData() const
	{
		...
		tempColliderData.myLayerID = &myColliderID;
		tempColliderData.myColliderIndex = GetGameObject().GetComponentIndex(*this);
	...
	void ColliderComponent::ToJson() const
{
		SB::JsonSerializer& serializer = SB::Engine::GetJsonSerializer();
		serializer.WriteObject("properties");

		serializer.WriteNode("Radius", myCollider.GetRadius());
		serializer.WriteNode("Trigger", myIsTrigger);
		serializer.WriteNode("RenderCollider", myObject->GetComponentCount<RenderColliderComponent>() > 0);
		serializer.WriteNode("Center", myOffset);

		serializer.StepOut();
		serializer.WriteNode("type", "ColliderComponent");
	}
