...
namespace ENGINE_NAMESPACE
{
	...
	{
		myRenderTexture = std::make_unique<RenderTexture>(aNewWidth, aNewHeight, false);
	}

	void TextureDebugger::Render()
	{
		Engine::GetRenderer().GetSpriteRenderer().RenderFullscreenQuad(myRenderTexture->GetTexture());
		myRenderTexture->Clear(SB::Vector4f::Zero);
		const float width = static_cast<float>(720.f) * 0.25f;
	...
	{
		const float height = static_cast<float>(720.f) * 0.25f;
