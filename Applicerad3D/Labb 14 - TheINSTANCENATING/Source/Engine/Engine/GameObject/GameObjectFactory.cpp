...
namespace ENGINE_NAMESPACE
{
	...
	ObjectPtr GameObjectFactory::CreateObjectFromPrefab(Scene & aScene, const std::string & aIdentifier)
	{
		DataNode ObjectNode = myPrefabDataBase.GetGlobalPrefab(aIdentifier);

		ObjectPtr object = LoadGameObject(aScene, ObjectNode);
	...
	ObjectPtr GameObjectFactory::LoadGameObject(Scene & aScene, const DataNode aGameObjectProperties)
	{
		...
		{
			ObjectPtr childObject = LoadGameObject(aScene, aGameObjectProperties["children"][i]);
