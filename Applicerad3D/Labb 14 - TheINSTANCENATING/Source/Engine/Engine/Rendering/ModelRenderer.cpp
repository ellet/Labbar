...
namespace ENGINE_NAMESPACE
{
	...
	ModelRenderer::ModelRenderer()
	{
		...
		SetSkyBox("Assets/Skyboxes/skybox4_16f.dds");
	...
	void ModelRenderer::Render(const std::shared_ptr<Effect> & aEffect, const std::shared_ptr<GenericMesh> & aMesh, const InstanceData & aInstanceData)
	{
		...
		myMeshes[identifier]->AddInstance(aEffect == nullptr ? myEffect : aEffect, aInstanceData);
	}
	...
	void ModelRenderer::SetSkyBox(const std::string & aFilePath)
	{

		mySkybox = std::make_shared<TextureCube>(aFilePath.c_str());
	...
	void BatchEntry::AddInstance(const std::shared_ptr<Effect> & aEffect, const InstanceData & aInstanceData)
	{
