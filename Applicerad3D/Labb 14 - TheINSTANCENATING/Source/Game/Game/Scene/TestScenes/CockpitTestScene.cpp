...
CockpitTestScene::CockpitTestScene(SB::PostMasterState & aPostMasterState)
	: SB::Scene(aPostMasterState, "Cockpit Test Scene")
{
	CreateFactories();
	
	mySprite.SetTexture(SB::Engine::GetResourceManager().Get<SB::Texture>("textures/cockpitPlaceholder.dds"));
	CreateAndAddModel("Cockpit Model", "models/tga_companioncube/companion.fbx", SB::Vector3f(0.f, 0.f, 5.f));
...
void CockpitTestScene::CreateFactories()
{
	...
	//
	//ObjectPtr enemy = std::make_shared<GameObject>();
	//
	//ComponentPtr<ModelComponent> prettyModel(GetComponentFactory<ModelComponent>()->CreateObject());
	//std::shared_ptr<AssimpModel> model = Engine::GetResourceManager().Get<AssimpModel>(myEffect, "models/unitsphere/sphere.fbx");
	//
