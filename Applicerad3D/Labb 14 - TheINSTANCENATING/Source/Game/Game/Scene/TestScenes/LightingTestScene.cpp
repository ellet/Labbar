...
LightingTestScene::LightingTestScene(SB::PostMasterState & aPostMasterState)
	: SB::Scene(aPostMasterState, "Lighting Test Scene")
{
	PushCameraController(std::make_shared<SB::FreeSpaceCameraController>(5.f, 1.5f));

	std::shared_ptr<SB::AssimpModel> model = SB::Engine::GetResourceManager().Get<SB::AssimpModel>("Assets/Models/TestModelHead/K11_1415.fbx");
