...
#include "StuffsForLabs/LoadModel.h"
...
void CGameWorld::Init()
{
	...
	*/
	...
	myPrettyRalf.Init(Vector3f(0.f, 0.f, 0.f), nullptr);

	ModelStuff::LoadModel("Ralf.ply", myPrettyRalf);

	GameObjectsManager::SetCameraPosition(Vector3f( 0.0f, 5.0f, 8.0f));

	GameObjectsManager::IncreaseCameraYaw(DEGRESS_TO_RADIANSF(180.f));
}
...
void CGameWorld::Update(float aTimeDelta)
{
	...
	{
		myPrettyRalf.RotateY(0.5f * aTimeDelta);
	}
	if (myInput.GetKeyDown(DIK_G))
	{
		myPrettyRalf.RotateX(0.5f * aTimeDelta);
	}
	if (myInput.GetKeyDown(DIK_B))
	{
		myPrettyRalf.RotateZ(0.5f * aTimeDelta);
	}
	...
	BIGLAMP::RotateLight(aTimeDelta);
}
