...
void ShapeContainer::AddVoxels(const CommonUtilities::GrowingArray<Voxel , unsigned int>& someVoxels)
{
	myVoxels = someVoxels;
}
...
void ShapeContainer::AddVoxels(const CommonUtilities::GrowingArray<VoxelData, unsigned int > &someVoxels, float aSize)
{
	Vector3f offset(0.5f, 0.5f, 0.5f);
	for (unsigned int i = 0; i < someVoxels.Size(); ++i)
	{
