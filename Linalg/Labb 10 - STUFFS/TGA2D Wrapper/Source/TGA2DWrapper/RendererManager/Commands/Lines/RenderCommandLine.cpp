...
void RenderCommandLine::Render(const Vector2f &aParentPosition, const Vector2f &aParentSize)
{
	...
	DX2D::CEngine::GetInstance()->GetDebugDrawer().DrawLine(
		DX2D::Vector2f((myStartPosition.x / virtualScreenSize.x)   + aParentPosition.x, (myStartPosition.y / virtualScreenSize.y) + aParentPosition.y),
		DX2D::Vector2f((myEndPosition.x / virtualScreenSize.x)   + aParentPosition.x, (myEndPosition.y / virtualScreenSize.y) + aParentPosition.y),
