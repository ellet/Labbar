...
void LoadModel(const std::string &aFile, Model& aModel) // loads a .ply model blenders obj format
{
	...
	for (int i = 0; i < nrOfVertexes; ++i)
	{
		Voxel newVoxel(CommonUtilities::Vector3<float>(floatProperties["x"][i], floatProperties["z"][i], floatProperties["y"][i])*0.5f
			, CommonUtilities::Vector3<float>(float(uCharProperties["red"][i])/255.0f, float(uCharProperties["green"][i])/255.0f, float(uCharProperties["blue"][i])/255.0f),
			CommonUtilities::Vector3<float>(floatProperties["nx"][i], -floatProperties["nz"][i], floatProperties["ny"][i]));
