...
namespace ModelStuff
{
	...
	void LoadModel(const std::string &aFile, ShapeContainer& aModel) // loads a .ply model blenders obj format
	{
		...
		CommonUtilities::GrowingArray<VoxelData, unsigned int> prettyVoxels;
		prettyVoxels.Init(9004);
		...
		for (unsigned int i = 0; i < nrOfVertexes; ++i)
		{
			VoxelData prettyVoxel;
			prettyVoxel.myPosition = Vector3f(floatProperties["x"][i],
				floatProperties["z"][i],
				floatProperties["y"][i]) *0.6f;

			prettyVoxel.myColour = Vector3<float>(float(uCharProperties["red"][i]) / 255.0f,
				float(uCharProperties["green"][i]) / 255.0f,
				float(uCharProperties["blue"][i]) / 255.0f);

			prettyVoxel.myNormal = Vector3<float>(floatProperties["nx"][i],
				-floatProperties["nz"][i],
				floatProperties["ny"][i]);

			prettyVoxels.Add(prettyVoxel);
		}
		aModel.AddVoxels(prettyVoxels);
	}

}

/*Voxel newVoxel(
CommonUtilities::Vector3<float>(floatProperties["x"][i],
floatProperties["z"][i],
floatProperties["y"][i])*0.5f


, CommonUtilities::Vector3<float>(float(uCharProperties["red"][i]) / 255.0f,
float(uCharProperties["green"][i]) / 255.0f,
float(uCharProperties["blue"][i]) / 255.0f),


CommonUtilities::Vector3<float>(floatProperties["nx"][i],
-floatProperties["nz"][i],
floatProperties["ny"][i])
);
