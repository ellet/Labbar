...
public:     // Sets line to (0,0) -> (0,0) as this will produce most errors making sure the
	...
	//
	Line(const Point2<T> &aFirstPoint, const Point2<T> &aSecondPoint)
	{
	...
	//
	void SetWith2Points(const Point2<T> &aFirstPoint, const Point2<T> &aSecondPoint)
	{
		myPoint = aFirstPoint;
	...
	//
	void SetWithPointAndDirection(const Point2<T> &aPoint, const Vector2<T> &aDirection)
	{
		myPoint = aPoint;
	...
	//
	bool IsInside(const Point2<T> &aPoint) const
	{
