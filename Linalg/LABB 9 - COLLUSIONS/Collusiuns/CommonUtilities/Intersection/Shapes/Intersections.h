...
namespace Intersection
{
	...
	bool KnownPointVsLineSegment(Vector3f aPoint, const LineSegment3D & aLine)
	{
		const float LineLength = (aLine.myEndPos - aLine.myStartPos).Length2();
	...
	bool SweptSphereVsAABB(LineSegment3D aLine, const float aRadius, const AABB & aAABB)
	{
		Vector3f lineDirection = (aLine.myEndPos - aLine.myStartPos).Normalize();

		Vector3f distanceFromStartToCenter = aAABB.myCenterPos - aLine.myStartPos;

		Vector3f closestPointOnLineToCenter = lineDirection * lineDirection.Dot(distanceFromStartToCenter);

		if (KnownPointVsLineSegment(closestPointOnLineToCenter, aLine) == false)
		{
			Vector3f distanceFromEndToCenter = aAABB.myCenterPos - aLine.myEndPos;
			if (distanceFromEndToCenter.Length2() > distanceFromStartToCenter.Length2())
			{
				closestPointOnLineToCenter = aLine.myStartPos;
			}
			else
			{
				closestPointOnLineToCenter = aLine.myEndPos;
			}
		}

		Vector3f DirectionToLine = (aAABB.myCenterPos - closestPointOnLineToCenter);

		if (DirectionToLine.Length2() < (aRadius * aRadius))
		{
			return true;
		}

		Vector3f ToMoveLineVector = DirectionToLine.GetNormalized() * aRadius;

		aLine.myEndPos += ToMoveLineVector;
		aLine.myStartPos += ToMoveLineVector;

        return LineVsAABB(aLine, aAABB, Vector3f());

		/*AABB temp = aAABB;

		aAABB.myMaxPos.x += aRadius;
		aAABB.myMaxPos.z += aRadius;
		aAABB.myMaxPos.y += aRadius;

		aAABB.myMinPos.x -= aRadius;
		aAABB.myMinPos.z -= aRadius;
		aAABB.myMinPos.y -= aRadius;

		return LineVsAABB(aLine, aAABB, Vector3f());


		if (LineVsAABB(aLine, aAABB, Vector3f()) == true)
		{
			if (CompareRadiusOfLineToDistanceOfPoint(aLine, temp.myMinPos, aRadius) == true ||
				CompareRadiusOfLineToDistanceOfPoint(aLine, temp.myMinPos, aRadius) == true || 
				CompareRadiusOfLineToDistanceOfPoint(aLine, temp.myMinPos, aRadius) == true || 
				CompareRadiusOfLineToDistanceOfPoint(aLine, temp.myMinPos, aRadius) == true || 
				CompareRadiusOfLineToDistanceOfPoint(aLine, temp.myMaxPos, aRadius) == true || 
				CompareRadiusOfLineToDistanceOfPoint(aLine, temp.myMinPos, aRadius) == true || 
				CompareRadiusOfLineToDistanceOfPoint(aLine, temp.myMinPos, aRadius) == true || 
				CompareRadiusOfLineToDistanceOfPoint(aLine, temp.myMinPos, aRadius) == true)
			{
