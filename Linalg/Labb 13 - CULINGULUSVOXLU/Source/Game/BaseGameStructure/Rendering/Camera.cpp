...
#include <Intersection/Shapes/Fov90Frustum.h>
	...
	const Vector3f &aPosition)
{
	...
	*/
	...
	myProjection = Matrix44f::CreateProjectionMatrixLH(0.1f, 100.f, aProjectionWidth/aProjectionHeight, DEGRESS_TO_RADIANS(90.f));

	myFrustum = new Intersection3D::Fov90Frustum(0.1f, 100.f);
...
Matrix44f Camera::GetInverse() const
{
	...
	return myTransformation.GetInverse();
}
...
const Intersection3D::Fov90Frustum & Camera::GetFrustum()
{
	return *myFrustum;
}
