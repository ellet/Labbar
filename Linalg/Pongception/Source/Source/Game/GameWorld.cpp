...
void CGameWorld::Draw()
{
	...
	DX2D::Vector4f tempVector(0.f, 0.f, 1.f, 1.f);
	for (unsigned int i = 0; i < myGameObjects.size(); ++i)
	{
		SetSprites(myGameObjects[i], tempVector, static_cast<unsigned int>((myRecursionLevel -1)));
	}

	for (size_t iGameObject = 0; iGameObject <= mySpriteID ; ++iGameObject)
	{
...
void CGameWorld::SetSprites(const GameObject & aObjectToDraw, DX2D::Vector4f aWorldData, unsigned int aDepth)
{
	//Do stuff
	...
	DX2D::Vector2f spriteSizef = DX2D::Vector2f(static_cast<float>(discardSpriteSize.x), static_cast<float>(discardSpriteSize.y));
	...
	ScaleToDraw.x *= aWorldData.z;
	ScaleToDraw.y *= aWorldData.w;

	mySprites[mySpriteID].SetSize(ScaleToDraw);

	const DX2D::Vector2f positionOfObject = aObjectToDraw.GetPostion();
	const DX2D::Vector2f thisWorldZero = DX2D::Vector2f(aWorldData.x, aWorldData.y);

	DX2D::Vector2f PositionToSet(thisWorldZero.x + (positionOfObject.x * aWorldData.z), thisWorldZero.y + (positionOfObject.y * aWorldData.w));

	mySprites[mySpriteID].SetPosition(PositionToSet);

	DX2D::Vector4f newWorld(PositionToSet.x, PositionToSet.y, ScaleToDraw.x, ScaleToDraw.y);
	++mySpriteID;
	if (aDepth > 0)
	{
		for (unsigned int i = 0; i < myGameObjects.size(); ++i)
		{
			SetSprites(myGameObjects[i], newWorld, (aDepth - 1));
		}
	}
}
