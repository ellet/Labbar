...
public:     // Sets all plane values to 0 to make it obvious to the user that it's not initialized.
			...
			Plane(void)
			{	
				myPoint = Point3<T>(0, 0, 0);
				myNormal = Vector3<T>(0, 0, 0);
			}

			// Initializes the plane using three points
			//
			Plane(const Point3<T> &aFirstPoint, const Point3<T> &aSecondPoint, const Point3<T> &aThirdPoint)
			{
			...
			//
			Plane(const Point3<T> &aPoint, const Vector3<T> &aNormal)
			{
			...
			//
			void SetWith3Points(const Point3<T> &aFirstPoint, const Point3<T> &aSecondPoint, const Point3<T> &aThirdPoint)
			{
				Vector3<T> firstVector = aThirdPoint - aFirstPoint;
				Vector3<T> secondVector = aSecondPoint - aFirstPoint;
			...
			//
			void SetWithPointAndNormal(const Point3<T> &aPoint, const Vector3<T> &aNormal)
			{
				myNormal = aNormal;
				myPoint = aPoint;
			}
			...
			//
			bool IsInside(const Point3<T> &aPoint) const
			{
