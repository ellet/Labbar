...
namespace CommonUtilities
{
	...
	void CommonUtilities::Heap<T, TComparer>::ClearAndSort(const CommonUtilities::GrowingArray<T> & aGrowingArray)
	{
		...
		unsigned short startValue = static_cast<unsigned short>(ceil(static_cast<float>(aGrowingArray.Size()) / 2.f - 1.0f));

		for (unsigned short i = startValue; i < aGrowingArray.Size(); --i)
		{
			Drop(myData[i]);
		}
	}
