...
CGame::CGame()
{
	...
	CU::SingletonInputWrapper::Create();
}
...
void CGame::Init(const std::wstring& aVersion)
{
	...
	myWindowSize.x = windowWidth;
	myWindowSize.y = windowHeight;
...
{
	CU::SingletonInputWrapper::Initialize(DX2D::CEngine::GetInstance()->GetHInstance(), *DX2D::CEngine::GetInstance()->GetHWND());
	myGameWorld.Init(myWindowSize);
}


void CGame::UpdateCallBack()
{
	CU::SingletonInputWrapper::Update();
	myGameWorld.Update();
