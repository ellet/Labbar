...
namespace HUG
{
	...
	void Renderer::DrawLine(const PixelPositionFloat & aStartPoint, const PixelPositionFloat & aEndPoint)
	{
		...
		const DX2D::Vector2f Start(tempStart.x, tempStart.y);
		const DX2D::Vector2f End(tempEnd.x, tempEnd.y);

		DX2D::CEngine::GetInstance()->GetDebugDrawer().DrawLine(Start, End);
	}
}