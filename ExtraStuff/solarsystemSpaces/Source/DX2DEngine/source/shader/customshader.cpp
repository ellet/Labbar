...
bool Tga2D::CCustomShader::PostInit(const char* aVertex, const char* aPixel, EShaderDataBufferIndex aBufferRegisterIndex)
{
	if (myCurrentDataIndex < 0)
	{
		ERROR_PRINT("Tga2D::CCustomShader::PostInit - No data to init with!");
		return false;
	}
	if (aBufferRegisterIndex <= EConstantBufferSlot_MAX - 1)
	{
		ERROR_PRINT("Tga2D::CCustomShader::PostInit - buffer index can not be below the standard buffers in index!");
		return false;
	}
	...
	commonBufferDesc.ByteWidth = sizeof(Tga2D::Vector4f) * (myCurrentDataIndex+1);
	...
	if (result != S_OK)
	{
		ERROR_PRINT("Tga2D::CCustomShader::PostInit - Size missmatch between CPU and GPU(shader)");
		return false;
	}
...
bool Tga2D::CCustomShader::Render(CRenderObjectSprite* aObject, ID3D11Buffer* aObjectBuffer, ID3D11Buffer* aVertexBuffer)
{
	if (!myIsPostInited)
	{
		ERROR_PRINT("Tga2D::CCustomShader::Render - Custom shader is not inited! Run PostInit() on this shader!");
		return false;
	}
...
void Tga2D::CCustomShader::SetShaderdataFloat4(Tga2D::Vector4f someData, EShaderDataID aID)
{
	...
	{
		ERROR_PRINT("Tga2D::CCustomShader::SetShaderdataFloat4() The id is bigger than allowed size");
		return;
	}
...
void Tga2D::CCustomShader::SetTextureAtRegister(Tga2D::CTexture* aTexture, EShaderTextureSlot aRegisterIndex)
{
	if (aRegisterIndex < 4)
	{
		ERROR_PRINT("Tga2D::CCustomShader::SetTextureAtRegister - Index below 4 is not allowed as they are reserve.");
		return;
	}
	if (myCurrentTextureIndex >= MAX_SHADER_TEXTURES)
	{
		ERROR_PRINT("Tga2D::CCustomShader::SetTextureAtRegister - Too many textures already bound!");
		return;
	}
