...
PixelOutput PShader(PixelInput aInput)
{
	...
	gAlbedo = gAlbedo * aInput.instanceColor.xyz;
    ...
    //Directional
    {
        float3 lightSpacePos = mul(directionLight[0].worldToLightSpace, aInput.worldPosition).xyz;
		float2 uv = lightSpacePos.xy * float2(0.5f, -0.5f) + float2(0.5f, 0.5f);
		bool isInside = uv.x >= 0.0f && uv.x < 1.0f && uv.y < 1.0f && uv.y <= 1.0f && lightSpacePos.z <= 1.0f;
        float depthValue = boundShadowMap.Sample(samplerState, uv).x;

        if (depthValue + 0.00001f >= lightSpacePos.z || isInside == false)
        {
            gLambert = saturate(dot(gObjectNormal, gToLight));
    
            directDiffuse += CalulateDirectDiffuse(directionLight[0].color.xyz) * directionLight[0].intensity;
            directSpecularity += CalulateDirectSpecularity(directionLight[0].color.xyz) * directionLight[0].intensity;
        }
    }
	...
	output.color.a = 1.0f;
    output.velocity.rg = aInput.viewPosition.xy - aInput.previousViewPosition.xy;
	output.velocity.rg /= 2.0f;
	output.velocity.g *= -1.0f;
    output.velocity.ba = float2(0.0f, 1.0f);
	return output;
}
