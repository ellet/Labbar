...
public:
	...
	char* myVerticies;
	struct aiMetadata *metaData;
	float minDistance;
	float maxDistance;
	bool hasMinMaxDistance;
};
...
private:
	void LoadNode(CLoaderModel* someInput, const struct aiScene * aScene, const struct aiNode * aNode, float aMinDist = -1.0f, float aMaxDist = -1.0f);
