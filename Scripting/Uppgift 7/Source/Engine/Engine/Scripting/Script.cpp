...
namespace SB
{
	...
	Script::Script(lua_State* aState, ScriptFile & aScriptFile, int aLuaRef)
	{
		L = aState;
	...
	void Script::MakeOrphan()
	{
		myScriptFile = nullptr;
	}

	void Script::Push(const float aValue)
	{
		myArgumentCount++;
		lua_pushnumber(L, aValue);
	}

	void Script::Push(const int aValue)
	{
		myArgumentCount++;
		lua_pushinteger(L, aValue);
	}

	void Script::Push(const char *aValue)
	{
		myArgumentCount++;
		lua_pushstring(L, aValue);
	}

	void Script::Push(const std::string &aValue) 
	{
		Push(aValue.c_str());
	}
	...
	void Script::PushArguments()
	{

	}

	void Script::CallFunction(const char *aFunctionName)
	{
		...
		if (lua_isfunction(L, 1))
		{
			...
			{
				Engine::GetLogger().LogError("Lua runtime error in file {0}: {1}", myPath, lua_tostring(L, -1));
				lua_pop(L, 1);
			}
		}
