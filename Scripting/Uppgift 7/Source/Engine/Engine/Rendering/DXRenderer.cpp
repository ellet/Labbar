...
namespace SB
{
	DXRenderer::DXRenderer()
	{
		...
		myAdditiveBlendingState = nullptr;
		myDepthStencilState = nullptr;
		myD3dDebug = nullptr;
		myAnnotationWriter = nullptr;
	}
	...
	DXRenderer::~DXRenderer()
	{
		...
		SAFE_RELEASE(myAdditiveBlendingState);
		SAFE_RELEASE(myDepthStencilState);
		SAFE_RELEASE(myAnnotationWriter);
	...
	void DXRenderer::Initialize(void *aOutputTarget, int aWidth, int aHeight, bool aFullscreen)
	{
		...
		CheckDXError(
			myDeviceContext->QueryInterface(__uuidof(*myAnnotationWriter), reinterpret_cast<void**>(&myAnnotationWriter))
		);
		...
		CheckDXError(
			myDevice->CreateRasterizerState(&rasterDesc, &myBackfaceRasterState)
		);
		...
		CheckDXError(
			myDevice->CreateBlendState(&blendStateDescription, &myAdditiveBlendingState)
		);
		...
		myCameraConstantBuffer = std::make_unique<ConstantBuffer<CameraConstantBuffer>>();	
	}

	void DXRenderer::ClearFrame()
	{
		myCameraConstantBuffer->BindToVS(0);
		myCameraConstantBuffer->BindToPS(0);
		myCameraConstantBuffer->BindToGS(0);
	...
	ID3DUserDefinedAnnotation* DXRenderer::GetAnnotationWriter() const
	{
		return myAnnotationWriter;
	}
	...
	PostProcessor & DXRenderer::GetPostProcessor()
	{
	...
	void DXRenderer::CreateBuffers(int aWidth, int aHeight)
	{
		...
		myPostProcessor = std::make_unique<PostProcessor>(aWidth, aHeight);
	...
	void DXRenderer::DisableBackfaceCulling()
	{
		myDeviceContext->RSSetState(myBackfaceRasterState);
	}


	void DXRenderer::EnableBackfaceCulling()
	{
		myDeviceContext->RSSetState(myRasterState);
	}


	void DXRenderer::EnableAdditiveBlendState()
	{
		float blendFactor[4];
		...
		myDeviceContext->OMSetBlendState(myAdditiveBlendingState, blendFactor, 0xffffffff);
	}


	void DXRenderer::DisableAdditiveBlendState()
	{
		float blendFactor[4];
	...
	void DXRenderer::DoFullscreenEffects(const std::shared_ptr<WorldRenderTexture> & aWorldTexture)
	{
		DisableDepthWrite();

		if (Engine::GetEngineSettings().GetUsePostProcessing() == true)
		{
			unsigned int effects = static_cast<unsigned int>(eEffectBundles::eNone);

			if (Engine::GetEngineSettings().GetUseBloom() == true)
			{
				effects = effects | static_cast<unsigned int>(eEffectBundles::eBloom);
			}
			
			if (Engine::GetEngineSettings().GetUseHDR() == true)
			{
				effects = effects | static_cast<unsigned int>(eEffectBundles::eHDR);
			}

			if (Engine::GetEngineSettings().GetUseMotionBlur() == true)
			{
				effects = effects | static_cast<unsigned int>(eEffectBundles::eMotionBlur);
			}

			myPostProcessor->DoFullscreenEffects(aWorldTexture, effects, 0);
		}
	...
	void DXRenderer::SetFullscreen(const bool aTrueFalse)
	{
		Resize(Engine::GetEngineSettings().GetResolution().x, Engine::GetEngineSettings().GetResolution().y);

		mySwapchain->SetFullscreenState(aTrueFalse, nullptr);
	}
	...
	{
		BOOL temp;
		mySwapchain->GetFullscreenState(&temp, nullptr);
		return (temp == TRUE);
	}

	void DXRenderer::SetCamera(const CameraConstantBuffer& aCameraData)
	{
		myCameraConstantBuffer->UpdateData(aCameraData);
	}
