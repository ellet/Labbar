	...
	SB
{
	void StreakComponent::Initialize()
	{
		...
		travelledTime = 0.f;
		myTrailingForwardPos = Vector3f::Zero;
		myTrailingForwardVector = Vector3f::Zero;
		myIsEmitting = true;
	...
	void StreakComponent::EndUpdate(const Time& aDeltaTime)
	{
	...
	void StreakComponent::Render(const Camera & aCamera) const
	{
	...
	void StreakComponent::ToJson(JsonSerializer &aSerializer) const 
{
		...
		myStreakLogic->SaveToJson(aSerializer);
	...
	void StreakComponent::EnableEmmision()
	{
		myIsEmitting = true;
	}

	void StreakComponent::DisableEmmision()
	{
		myIsEmitting = false;
	}
