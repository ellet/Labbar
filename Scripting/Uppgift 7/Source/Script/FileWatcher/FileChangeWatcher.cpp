...
namespace Scripting
{
	...
	void FileChangeWatcher::CheckChanges(std::vector<std::string> & aListOfChangedFilesToBuild)
	{
		...
		for (int i = 0; i < static_cast<int>(myChanges.size()); i++)
		{
			...
			{
				aListOfChangedFilesToBuild.push_back(myChanges[i].file);
	...
	/*bool FileChangeWatcher::PostChanges()
	{
		...
		for (int i = 0; i < static_cast<int>(myChanges.size()); i++)
		{
			if (myChanges[i].postTime <= myWatch.GetElapsedTime().InSeconds())
			{
				PMPostMaster::Post(FileChangedEvent(myChanges[i].file));
				myChanges.erase(myChanges.begin() + i);
				i--;
			}
		}
