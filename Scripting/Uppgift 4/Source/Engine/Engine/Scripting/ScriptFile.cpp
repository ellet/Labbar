...
namespace SB
{
	...
	std::string ReadFile(const char * aFile)
	{
		...
		if (t.good() == false)
		{
			Error("Tried to read file " + std::string(aFile) + ", which doesn't exist. Is the path entered correctly?");
		}
		std::string str;
	...
	{
		std::shared_ptr<Script> script = std::make_shared<Script>(L, *this, ExecuteAndGetRef());
