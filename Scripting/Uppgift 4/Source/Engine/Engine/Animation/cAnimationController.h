...
using mat4 = SB::Matrix44f;
using vec4 = SB::Vector4f;
using quat = SB::Quaternion;
...
public:
	...
	const std::vector<mat4>& GetTransforms(float dt) const { return Transforms[GetFrameIndexAt(dt)]; }
	unsigned int GetFrameIndexAt(float time) const;
...
public:
	...
	const std::vector<mat4>& GetTransforms(int aAnimationIndex, float aTime) const { return Animations[aAnimationIndex].GetTransforms(aTime); } 

	int32_t GetAnimationIndex(const char * aName) const;
