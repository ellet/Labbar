...
namespace SB
{
	...
	void Sprite::SetPositionInNormalizedSpace(const Vector2f & aPosition)
	{
		Vector2f renderSize = GetRenderSize();
		Vector2f pos = aPosition;
		pos.y *= -1.0f;
		pos += Vector2f::One;
		pos /= 2.0f;
		SetPosition(pos * renderSize);
	}
	...
	{
		return Vector2f(1080.f * (Engine::GetRenderer().GetRenderTargetResolution().x / Engine::GetRenderer().GetRenderTargetResolution().y), 1080.f);
	}

	SB::Vector4f Sprite::GetBorderPositions() const
	{
