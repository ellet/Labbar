...
namespace SB
{
	...
	SpriteEffect::SpriteEffect(const char * aVertexFile, const char * aVertexEntryPoint, const char * aPixelFile, const char * aPixelEntryPoint)
	{
		...
		layout.AddPerInstance("TEXTOPLEFT", 0, DXGI_FORMAT_R32G32_FLOAT, 1);
		layout.AddPerInstance("TEXSIZE", 0, DXGI_FORMAT_R32G32_FLOAT, 1);

		AttachVertexShader(aVertexFile, aVertexEntryPoint, layout);
		AttachPixelShader(aPixelFile, aPixelEntryPoint);
	}

	SpriteEffect::SpriteEffect(const char * aPixelFile, const char * aPixelEntryPoint)
		: SpriteEffect("shaders/sprite/sprite.fx", "VShader", aPixelFile, aPixelEntryPoint)
	{
	}
