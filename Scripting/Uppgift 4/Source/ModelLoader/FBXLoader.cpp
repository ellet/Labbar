...
int CFBXLoader::DetermineAndLoadVerticies(aiMesh* fbxMesh, CLoaderMesh* aLoaderMesh)
{
	...
	vertexBufferSize += (fbxMesh->HasBones() ? (sizeof(SB::Vector4f) + sizeof(SB::Vector4ui)) : 0); // Better with an UINT, but this works
	...
	for (unsigned int i = 0; i < fbxMesh->mNumVertices; i++)
	{
		...
		if (fbxMesh->HasBones())
		{
			...
			//std::cout << boneData.IDs[0] << std::endl;
			vertexCollection.PushVec4(SB::Vector4ui(boneData.IDs[0], boneData.IDs[1], boneData.IDs[2], boneData.IDs[3]));
...
void CFBXLoader::LoadNode(CLoaderModel* someInput, const aiScene * aScene, const aiNode * aNode, float aMinDist, float aMaxDist)
{
	CLoaderModel* model = someInput;

	for (unsigned int n = 0; n < aNode->mNumChildren; ++n)
	{
		aiNode * node = aNode->mChildren[n];

		for (unsigned int i = 0; i < node->mNumMeshes; i++)
		{
			unsigned int index = node->mMeshes[i];
			CLoaderMesh* mesh = model->CreateMesh();
			aiMesh* fbxMesh = aScene->mMeshes[index];

			if (fbxMesh->mNumFaces > 0)
			{
				mesh->myMeshName = node->mName.C_Str();
			}


			for (unsigned int f = 0; f < fbxMesh->mNumFaces; f++)
			{
				for (uint j = 0; j < fbxMesh->mFaces[f].mNumIndices; j++)
				{
					mesh->myIndexes.push_back(fbxMesh->mFaces[f].mIndices[j]);
				}
			}

			DetermineAndLoadVerticies(fbxMesh, mesh);

			if (aMinDist >= -1.0f && aMaxDist >= 0.f)
			{
				mesh->hasMinMaxDistance = true;
				mesh->minDistance = aMinDist;
				mesh->maxDistance = aMaxDist;
			}
			else
			{
				mesh->hasMinMaxDistance = false;
			}
		}

		if (node->mMetaData != nullptr)
		{
			node->mMetaData->Get<float>("LOD_distStart", aMinDist);
			node->mMetaData->Get<float>("LOD_distEnd", aMaxDist);
		}

		LoadNode(someInput, aScene, node, aMinDist, aMaxDist);
	}
}
...
void* CFBXLoader::LoadModelInternal(CLoaderModel* someInput)
{
	...
	LoadNode(model, scene, scene->mRootNode);
