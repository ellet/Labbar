...
unsigned int cAnimEvaluator::GetFrameIndexAt(float ptime) const
{
...
int32_t SceneAnimator::GetAnimationIndex(const char * aName) const
{
	std::map<std::string, uint32_t>::const_iterator itr = AnimationNameToId.find(aName);
	return itr->second;
}
...
{
    dual[0].x = q.GetXYZW().x ; 
    dual[0].y = q.GetXYZW().y ; 
    dual[0].z = q.GetXYZW().z ; 
    dual[0].w = q.GetXYZW().w ; 
    dual[1].x = 0.5f *  ( tran.x * q.GetXYZW().w + tran.y * q.GetXYZW().z - tran.z * q.GetXYZW().y ) ; 
    dual[1].y = 0.5f *  (-tran.x * q.GetXYZW().z + tran.y * q.GetXYZW().w + tran.z * q.GetXYZW().x ) ; 
    dual[1].z = 0.5f *  ( tran.x * q.GetXYZW().y - tran.y * q.GetXYZW().x + tran.z * q.GetXYZW().w ) ; 
    dual[1].w = -0.5f * ( tran.x * q.GetXYZW().x + tran.y * q.GetXYZW().y + tran.z * q.GetXYZW().z ) ; 
}
