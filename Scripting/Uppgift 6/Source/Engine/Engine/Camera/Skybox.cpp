...
namespace SB
{
	Skybox::Skybox(const std::shared_ptr<Texture> & aTexture)
	{
		...
		mySurface.SetSlot(SurfaceTextureIndex::eDiffuse, aTexture);

		AddMesh(std::make_shared<Mesh<Vertex>>(mySurface, verticies, indicies));
	}
	...
	bool Skybox::MakeReady(bool aLoadAsynchronously)
	{
